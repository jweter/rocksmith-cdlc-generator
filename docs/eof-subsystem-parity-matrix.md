...
| Shift vs legato slide | PyGuitarPro `SlideType.shiftSlideTo`/`legatoSlideTo` | `guitarpro_import.py:_slide_kinds()`, `_resolve_slide_target_frets()` | PARITY | port | P1 | Both subtypes are distinguished at import via `slide_kinds`, and both now resolve/export the same way (see "Pitched slide" row above); no heuristic needed since PyGuitarPro's own parsed model already carries the distinction explicitly and RS XML's `slideTo` attribute does not distinguish shift from legato. |
