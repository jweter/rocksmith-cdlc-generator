...
| Bend strength | `rs.c` (RS XML export: `step` attribute = quarter-step count / 2.0, i.e. semitones), PyGuitarPro's own GP decoding (`gp3.py:readBend`, already normalizes raw GP bytes to `BendPoint.position` 0..12 and `.value` in whole semitones, `bendSemitone=25`/`bendPosition=60`) | `guitarpro_import.py:_bend_points()`, `source_import.SourceBendPoint` | PARTIAL | port | P0 | Import-side data preservation lands: the note's full bend curve (position fraction 0.0-1.0, semitone magnitude, vibrato) is now captured into `SourceNoteEvent.bend_points` instead of being discarded down to a boolean `"bend"` technique flag. PyGuitarPro already normalizes raw GP bytes to whole-semitone granularity, so EOF's internal quarter-step/half-step byte-encoding distinction (`rs.c`'s `bendstrength & 0x80` branch) is an EOF-internal storage-format detail this project's importer never needs to replicate. Remaining scope: Rocksmith XML export of the captured curve (propagate through `reviewed_export_events.py`, add `bend` to `rocksmith_xml.py`'s `DIRECT_NOTE_TECHNIQUES`, emit `<bendValues>`/`<bendValue time="..." step="..."/>` elements) -- `bend` is still excluded from `DIRECT_NOTE_TECHNIQUES`, so a note with a bend still fails closed at the reviewed-XML boundary rather than being silently exported without it. |
| Bend curve points | GP bend structures / RS | technique model | UNASSESSED | port | P1 | Determine fidelity needed for RS2014. |
| Slide in from above/below | `src/gp_import.c` reads the GP slide-type byte but discards it (no EOF import-side mapping exists, confirmed at commit `c0d88eabf7b00b0bd2cac9414df9fa9c6b3e7100`); PyGuitarPro `guitarpro.models.SlideType` (`intoFromAbove`/`intoFromBelow`) used instead | `guitarpro_import.py:_slide_kinds()`, `SourceNoteEvent.slide_kinds` | PARTIAL | study | P0 | Import-side data preservation lands: all six PyGuitarPro `SlideType` subtypes (two "into", two "out", shift, legato) are now captured per-note in an additive `slide_kinds` field instead of being collapsed into one generic `"slide"` technique flag. That generic flag is kept unchanged (existing validation in `eof_rocksmith_validation.py`/`reviewed_techniques.py` already depends on its exact string), so `slide_kinds` is a separate, additive field rather than a `techniques` label to avoid tripping the `SUPPORTED_TECHNIQUES` whitelist those modules enforce. EOF audit finding: `gp_import.c` never maps the GP slide-type byte to any note flag (log-only), so there is no EOF import-time behavior to port here; the field is sourced from PyGuitarPro's own enum alone. EOF's actual Rocksmith-facing slide model (`EOF_PRO_GUITAR_NOTE_FLAG_SLIDE_UP`/`_SLIDE_DOWN`/`_UNPITCH_SLIDE`) is populated in `src/rs_import.c` from Rocksmith XML's `slideTo`/`slideUnpitchTo` attributes on re-import, not from GP import -- that is the relevant reference for the export slice. Remaining scope: resolving each pitched slide's target fret (GP encodes this implicitly as the next same-string note, not as an explicit value PyGuitarPro exposes) and Rocksmith XML export (`slideTo`/`unpitchSlideTo` attributes). |
