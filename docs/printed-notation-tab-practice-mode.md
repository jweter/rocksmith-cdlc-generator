...
1. **Validation** — reuse the `eof_rest_boundary_check.py`/`eof_short_note_truncation_check.py` advisory-check pattern for rest/sustain correctness against the arrangement `build_printed_notation_bass_xml()` produces; add a click-to-measure-boundary alignment check consuming the same tempo map (the doc's explicit acceptance criterion under "First proof of concept"). Remember `click_track_render.count_in_offset_seconds(tempo_map, count_in_measures)` when pairing a rendered click WAV with this XML's chart timestamps as combined practice audio — the tempo map and the WAV do not share a clock on their own (PR #477).
2. **Image intake reuse** — register the source page image via the existing `official_tab_reference.py` (hashing, dedupe, per-measure region mapping, rotation), which already solves photo intake/provenance for issue #453's TAB viewer, even though this slice has no OCR to drive automatic region detection (regions are hand-tagged in the fixture for now).
3. **CLI** — `import-notation` subcommand in `cli.py`, mirroring `import-gp`/`import-musicxml` (`project`, source-file flag, `--instrument bass`), wired to `printed_notation_authoring.build_printed_notation_bass_xml()` (already landed) plus an XML-write step (see `desktop_xml_export.py`/`eof_cli.py` for the existing write-to-file convention).
4. **Docs** — update this section once the above lands.
