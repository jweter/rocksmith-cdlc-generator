...
- `import-notation` CLI subcommand in `cli.py` (mirrors `import-gp`/`import-musicxml`'s `project`/source-file-flag/instrument-role shape) wired to a new orchestration function, `printed_notation_authoring.import_project_printed_notation_practice()`: fixture -> canonical import -> `ReviewedExportArrangement` -> sustain-boundary check -> Rocksmith Bass XML -> click-track WAV -> click-alignment check, writing `arr_bass_RS2.xml`, `click.wav`, `sustain_report.json`, and `click_alignment_report.json` under `PROJECT/printed_notation/`. Fails closed on either validation check (raises instead of writing a possibly-broken practice package), per the doc's "unresolved low-confidence recognition blocks final promotion" and "no sustain crosses ... an incompatible subsequent event" requirements. An optional `--page-image` registers the source page as private reference evidence via `official_tab_reference.py` (`register_printed_notation_page_image()`, single-page fixtures only) — reusing issue #453's existing hash/dedupe/copy-into-project image store rather than inventing a second one, satisfying the "Image intake reuse" plan item without needing OCR-driven automatic region detection.
- Tests: `tests/test_cli_import_notation.py` (2 cases: required/optional argument parsing) and additions to `tests/test_printed_notation_authoring.py` (writes-all-four-outputs happy path, fails-closed-on-unreviewed-events, page-image registration end-to-end, measure-range mapping, multi-page rejection — 7 new cases, 16 total in that file).

These modules take the doc's "First proof of concept" acceptance target (one photographed bass-TAB page -> reviewed events -> tempo map -> count-in+click -> Rocksmith Bass arrangement -> validation -> playable practice PSARC ingredients) from a set of composable library functions to a single runnable command, end-to-end, against a fixture standing in for real recognition. They do not yet touch a desktop UI, or real image recognition.
...
### Proof-of-concept slice: complete

The narrow scope this "Implementation plan" section originally set out to build is now fully landed: a fixture standing in for real recognition can be turned into a validated, playable-shaped Rocksmith Bass practice XML plus a synchronized click-track WAV with one CLI command (`cdlc import-notation`), reusing this repository's existing reviewed-authoring, XML-rendering, and private-reference-image infrastructure at every step rather than forking new versions of any of it. Nothing past this point has been scoped as a committed next slice — the items below are candidate next directions, not an implementation plan; whoever picks this up next should write a fresh, concrete plan in this section before starting, following the same pattern this section has used throughout, rather than starting from the narrative sections above.

Candidate next directions, roughly in order of how directly they build on what exists:

- **Desktop UI wiring** — expose the same pipeline `import_project_printed_notation_practice()` already runs from a guided-desktop workspace (see `guided_desktop.py`/`desktop_xml_export_window.py` for the existing wiring pattern) instead of only the CLI.
- **Multi-page assembly** (doc phase N8) — `register_printed_notation_page_image()` and the fixture schema currently support exactly one page; multiple pages need page-ordering, continuity checks, and one registration call per page.
- **Generated backing audio beyond the click track** (doc phase N5) — deterministic drums/harmonic accompaniment layered onto `click_track_render.py`'s output.
- **Mid-song time-signature changes** — `TempoMap` only carries one top-level signature; `deterministic_tempo_map.py` would need a schema extension.
- **Real image recognition (OMR/OCR)** (doc phase N0) — still its own dedicated research spike (library/model evaluation, licensing, staff/TAB-line detection) before any of the above becomes usable from an actual photograph instead of a hand-authored fixture.
