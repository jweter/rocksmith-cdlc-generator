...
When fan-out includes Bass, only reconciliation/review artifacts that carry matching score SHA-256 and Bass track provenance may survive. Bass mapping and validation outputs do not yet carry enough independent source identity to prove that they were derived from that reconciliation, so fan-out invalidates `bass_mapped.json`, its mapping-review output, the validation report, validation flags, and review summary even when the reconciliation itself still matches. If reconciliation provenance does not match, reconciliation and source-disagreement artifacts are invalidated too. Lead/Rhythm-only fan-out does not disturb Bass work.
