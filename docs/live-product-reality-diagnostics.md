...
Diagnostic project lifecycle events are project-scoped. A `Project opened` event is recorded only after the requested project is actually the active project, and workflow-state deduplication includes project identity so switching between two projects with the same readiness text still records each project's initial state.
