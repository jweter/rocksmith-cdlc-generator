...
- **Last observed:** 2026-08-28 (Arrangement Preview tab, issue #454)
- **Status:** mitigated
- **GitHub references:** #193, #305, #304, #454, PR #454's fix
- **User-visible symptom:** On a laptop-resolution window with Windows display scaling, packaged Product Reality testing on 2026-08-28 confirmed the Arrangement Preview tab's content (score/fretboard controls plus, after loading an official TAB image, the TAB reference viewer) extended below the visible application window with no way to reach the rest of the page. Individual nested scrollbars inside the tab (for example the official TAB image canvas' own vertical/horizontal scrollbars) could only reach content *inside themselves*, not sibling controls packed elsewhere in the same tab.
- **Failing check / evidence:** User screenshot on 2026-08-28 plus issue #454's reproduction steps. The same underlying shape (packed content silently exceeding the visible tab area) was already fixed once for the Score & Mappings tab in issue #304, by reordering the human-confirmation controls ahead of a tall fixed-height table and giving that table and other unbounded text panels their own individual scrollbars (`tests/test_desktop_score_tab_layout.py`).
- **Root cause:** `ttk.Notebook` tab frames size to their available space, but plain `pack(fill="x", ...)` children inside a tab do not themselves scroll — they simply keep stacking vertically. The Arrangement Preview tab in particular is built incrementally by five separate mixins (`ArrangementPreviewSongWorkspaceWindow`, `EOFMeasureReviewMixin`, `LiveReviewEnhancementMixin`, `OfficialTabReferenceMixin`, plus whatever else layers onto `_build_arrangement_preview` next), each unaware of how much vertical space the others already used. #304's fix addressed the same *class* of defect locally (per-widget scrollbars on specific tall children) but did not add a page-level scroll region, so a tab whose *combined* mixin content exceeds the viewport — as Arrangement Preview's now does once the official TAB viewer is shown — was still unreachable.
- **Affected surfaces:** `arrangement_preview_ui.py`'s `_build_layout`, and by extension every mixin that packs into `self.arrangement_preview_tab` (`eof_measure_review_ui.py`, `live_review_enhancements.py`, `official_tab_reference_ui.py`). Any other notebook tab built the same incremental-mixin way is equally exposed.
- **Why prior safeguards missed it:** #304's fix proved out per-widget scrollbars for one already-tall child but did not establish a general page-level scroll convention, so it did not protect a *different* tab whose problem was the sum of many mixins' content rather than one tall child.
- **Corrective design pattern:** Wrap the entire notebook tab body in a `Canvas` + vertical `Scrollbar`, with the existing content frame re-parented onto the canvas via `create_window` instead of packed directly into the notebook. Every existing mixin keeps packing into the same named attribute (`self.arrangement_preview_tab`) unmodified; only its parent changed. Pin the embedded frame's width to the canvas viewport width on `<Configure>` so horizontal layout is unaffected, recompute the scroll region from the frame's own `<Configure>` (size-change) event, and scope global `<MouseWheel>` handling to `<Enter>`/`<Leave>` on the outer canvas so it never steals wheel events over another tab, a dialog, or a nested canvas with its own scrolling.
- **Fix applied:** `ArrangementPreviewSongWorkspaceWindow._build_scrollable_arrangement_preview_tab` in `arrangement_preview_ui.py` (issue #454).
- **Verification:** `python -m pytest -q` (1498 passed), `python -m compileall -q src tests`, `python scripts/check_automation_readiness.py`, and `python scripts/quality_preflight.py` all pass locally. `tests/test_arrangement_preview_scroll_ui.py` drives the real construction method against lightweight `tk`/`ttk` stand-ins (this repository's established no-live-display convention) and asserts the canvas/scrollbar wiring, scrollregion recompute, viewport-width pinning, and scoped mousewheel bind/unbind. Packaged Windows Product Reality confirmation that every control is reachable at the user's laptop resolution is still the human-gated acceptance step per issue #454.
- **Regression protection:** `tests/test_arrangement_preview_scroll_ui.py` covers the wiring; `tests/test_desktop_score_tab_layout.py` continues to cover the Score & Mappings tab's per-widget scrollbars.
...
- **Residual risk:** A future mixin added to `_build_arrangement_preview` still needs to remember it is packing into a scrollable frame (harmless by default, since the frame just grows) but should avoid assuming a fixed visible height. No other notebook tab has yet been audited for the same combined-mixin-height risk.
- **Prevention rule:** Any `ttk.Notebook` tab whose content is assembled by more than one mixin/override packing widgets vertically must either get a page-level `Canvas`+`Scrollbar` wrapper up front, or be explicitly re-audited for total height at the minimum supported window size every time a new mixin is added to it.
