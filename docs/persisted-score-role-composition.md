...
Adding/removing tracks from a role's selection (`plan-select`) and resolving cross-track overlaps remain `cdlc-score-composition` CLI-only workflows; an in-workspace track picker and overlap-decision UI, plus the downstream consumer that reads `review/score_role_composition_fanout.json` for a role instead of only the single primary-mapped track, remain open follow-on slices of issue #232.
