...
Resolving cross-track overlaps remains a `cdlc-score-composition overlaps`/`compose --decisions` CLI-only workflow; an in-workspace overlap-decision UI, plus the downstream consumer that reads `review/score_role_composition_fanout.json` for a role instead of only the single primary-mapped track, remain open follow-on slices of issue #232.
