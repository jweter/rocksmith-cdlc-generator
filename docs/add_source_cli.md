...
For sources attached to a project, `add-source` writes a hash-backed metadata receipt beneath `PROJECT/sources/intake/`. For completed imports, the receipt uses the source SHA-256 recorded by the workflow that actually completed (the project manifest for audio or imported-source provenance for symbolic/package sources) and refuses to write a receipt if the local source bytes changed during ingest. Receipt filenames use a bounded, normalized source stem so valid long source filenames cannot exceed common filesystem component limits. The receipt records the recognized format, route decision, source SHA-256, rights class, review state, and optional license note without copying additional source material into the repository. Default `projects/` remains gitignored.
