        ...
        if note.start + note.duration > duration + 0.05:
            items.append(
                ReviewItem(
                    ...
                    severity="FAIL",
                    stage="transcription",
                    message=f"Bass note {index} extends beyond the source audio.",
        ...
        if index > 0 and note.start < previous_end - 0.01:
            items.append(
                ReviewItem(
                    ...
                    severity="WARNING",
                    stage="transcription",
                    message=f"Bass note {index} overlaps the preceding monophonic note.",
                    time_seconds=note.start,
                    note_index=index,
                    priority=80,
                )
        ...
        if note.review_required or note.confidence < 0.55:
            items.append(
                ReviewItem(
                    ...
                    severity="WARNING",
                    stage="transcription",
                    message=f"Bass note {index} requires human review (confidence {note.confidence:.2f}).",
                    time_seconds=note.start,
                    note_index=index,
                    priority=85 if note.confidence < 0.40 else 65,
                )
            )
...
def validate_project_to_disk(project_dir: Path) -> Path:
    ...
    destination = project_dir / "review" / "validation_report.json"
