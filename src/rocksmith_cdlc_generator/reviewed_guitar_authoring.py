...
def _validated_guitar_note(
    ...
    if tuning[note.string_index] + note.fret != note.midi:
        raise ValueError(f"{role.value} source event {note.source_event_index} string/fret position does not match pitch")
...
def guitar_authoring_input_from_reviewed_export(
    ...
    notes = [_validated_guitar_note(note, tuning, arrangement.role) for note in arrangement.notes]
    chords = [_validated_chord(group) for group in arrangement.chord_groups]
