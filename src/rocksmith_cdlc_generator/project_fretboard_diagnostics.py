...
from .fretboard_candidate_inventory import FretboardCandidateInventory, build_fretboard_candidate_inventory
...
def build_project_fretboard_diagnostic(
    ...
    *,
    ...
    """Inspect the current human-confirmed fan-out without choosing or changing fingering.

    The diagnostic is bound to the current registered score, confirmed role mapping, and
    fan-out manifest/output. Any stale or mismatched authority fails closed before the
    pitch-correct candidate inventory is built.
    """
    ...
    entries = [entry for entry in manifest.arrangements if entry.role is role]
    if len(entries) != 1:
        raise ValueError(f"Current score fan-out must contain exactly one {arrangement} arrangement")
    entry = entries[0]
    if entry.source_track_index != mapping.source_track_index:
        raise ValueError(f"{arrangement} fan-out does not match the human-confirmed score track")
