...
For emerging compute, the current applicability state is LOW. Do not create quantum/experimental-compute work merely because the technology is novel. Require a measured classical bottleneck, credible mathematical mapping, verified classical baseline, identical musical/timing measurement contract, cheap reversible first experiment, operational justification, and independent verification. Otherwise classify the proposal as `NOT_JUSTIFIED` and continue classical engineering.
