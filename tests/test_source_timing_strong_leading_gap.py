...
def test_qualification_blocks_multiple_strong_bass_onsets_before_projected_score_edge(
    tmp_path: Path,
) -> None:
    ...
    score_pattern = [40, 43] * 4
    ImportedSource(
        provenance=SourceProvenance(
            source_type="guitarpro",
            source_filename="leading-gap.gp5",
            source_sha256="a" * 64,
            importer="fixture",
            importer_version="1",
        ),
        ...
        tracks=[
            SourceTrack(
                ...
                notes=[
                    ...
                    for index, midi in enumerate(score_pattern)
                ],
            )
        ],
    ...
    # Three reliable Bass onsets materially precede the projected first score
    # event, but their pitch estimates do not match the score edge. The later
    # equal-pitch pattern still matches perfectly at zero shift, so an
    # equal-pitch-only leading-edge guard would incorrectly allow promotion.
    audio_notes = [
        _audio(7.0, 52),
        _audio(8.0, 55),
        _audio(9.0, 57),
    ...
    assert "strong Bass onsets" in report.reason
    assert "EOF-derived" in report.reason
