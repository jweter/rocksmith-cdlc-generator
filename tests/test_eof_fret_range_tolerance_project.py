...
import rocksmith_cdlc_generator.eof_fret_range_tolerance_project as project_report
from rocksmith_cdlc_generator.eof_fret_range_tolerance_check import EOFFretRangeToleranceReport
from rocksmith_cdlc_generator.eof_fret_range_tolerance_project import (
    EOF_FRET_RANGE_TOLERANCE_REPORT_PATH,
    build_project_eof_fret_range_tolerance_report,
    write_project_eof_fret_range_tolerance_report,
)
...
def test_build_report_reads_the_projects_registered_score(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    project, _score = _project_with_score(tmp_path, monkeypatch)

    report = build_project_eof_fret_range_tolerance_report(project, instrument="bass")
...
def test_write_report_persists_it_under_the_project_review_directory(
    ...
    assert destination == project / EOF_FRET_RANGE_TOLERANCE_REPORT_PATH
    assert destination.is_file()
    assert score.read_bytes() == before
    persisted = EOFFretRangeToleranceReport.model_validate_json(
...
def test_rejects_a_directory_that_is_not_a_cdlc_project(tmp_path: Path) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="Not a CDLC project"):
        write_project_eof_fret_range_tolerance_report(not_a_project, instrument="bass")
